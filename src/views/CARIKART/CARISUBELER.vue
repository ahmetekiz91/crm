            ...
            <div class='row'>
              <div class='col-5'>
                <button type='button' class='btn btn-primary btn-sm' data-toggle='modal' data-target='#modal-xl' @click='Temizle()'>
                  <i class='fa fa-plus'></i> Cari Şube Ekle
                </button>
              </div>
              <div class='col-4'><h3 class=''><b>Cari Şubeler</b> </h3> </div>
              <div class='col-3'></div>
            </div>
          </div>
          ...
          <div class='card-body'>
            ...
            <div class="table-responsive">
              <table id='example1' class='table table-bordered table-striped'>
                ...
                <tbody>
                  <tr v-for='(n,index) in CARISUBELERList'>
                    <td>{{n.CARI_UNVAN}}</td>
                    <td>{{n.KODU}}</td>
                    <td>{{n.ADI}}</td>
                    <td>{{n.ADRES}}</td>
                    ...
                    <td>
                      <a class='btn btn-danger btn-sm sil text-white' @click='Delete(n.CARI_ADR_ID,index)'><i class='fas fa-trash'></i>Sil</a>
                      <a class='btn btn-primary btn-sm edit text-white' data-toggle='modal' data-target='#modal-xl' @click='Edit(n.CARI_ADR_ID,index)'><i class='fas fa-edit'></i>Güncelle</a>
                    </td>
                  </tr>
        ...
        <div class='modal-dialog modal-xl'>
          <div class='modal-content'>
              ...
              <div class='row'>
                ...
                <div class='col-md-12'>
                  ...
                  <div class='card card-primary'>
                    <div class='card-header'>
                      <h3 class='card-title'> {{ model.CARI_ADR_ID==null?'Cari Şube Ekle':'Cari Şube Düzenle' }}</h3>
                    </div>
                    <div class=''>
                      <div class='card-body'>
                        <div class="row">
            ...
            <div class='modal-footer justify-content-between'>
              <button type='button' class='btn btn-danger btn-sm w-25' data-dismiss='modal'>Kapat</button>
              <button type='button' class='btn btn-primary btn-sm w-25' v-on:click='(model.CARI_ADR_ID==null?Save():Update())'>Kaydet</button>
            </div>
          </div>
    ...
    methods:
    {
      ...
      async Save() {
        try {
          ...
          const res = await fetch(config.API_LOCATION + 'CARISUBELER', options);
          if (!res.ok) {
            const message = 'An error has occured: ${res.status} - ${res.statusText}';
            throw new Error(message);
          }
      ...
      async Edit(CARI_ADR_ID) {
        ...
        try {
          const res = await fetch(config.API_LOCATION + 'CARISUBELER?ID=' + CARI_ADR_ID, options);
          if (!res.ok) {
            const message = 'An error has occured: ${res.status} - ${res.statusText}';
            throw new Error(message);
          }
      ...
      async List()
      {
        ...
        try {
          ...
          const res = await fetch(config.API_LOCATION + 'CARISUBELER/getAll', options);
          if (!res.ok) {
            const message = 'An error has occured: ${res.status} - ${res.statusText}';
            throw new Error(message);
          }
          const data = await res.json();
          this.CARISUBELERList = data;
          this.setDatatable();
      ...
      async Deletemethod(CARI_GRUP_ID, index) {
        ...
        try {
          ...
          const res = await fetch(config.API_LOCATION + 'CARISUBELER?ID=' + CARI_GRUP_ID, options);
          if (!res.ok) {
            const message = 'An error has occured: ${res.status} - ${res.statusText}';
            throw new Error(message);
          }
          ...
          if (data == "OK")
          {
            this.CARISUBELERList.splice(index, 1);
            $('#example1').DataTable().destroy();
            this.setDatatable();
