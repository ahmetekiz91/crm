        ...
        <div class='card mt-2'>
          <div class='card-header'>
            <div class='row'>
              ...
              <div class='col-4'><h3 class=''><b>Cari Yetkililer</b> </h3> </div>
              <div class='col-3'></div>
            </div>
          </div>
          <!-- /.card-header -->
          <div class='card-body'>
            ...
            <div class="table-responsive">
              <table id='example1' class='table table-bordered table-striped'>
                ...
                <tbody>
                  <tr v-for='(n,index) in CARIKISILERList'>
                    <td>{{n.CARI_UNVAN}}</td>
                    <td>{{n.ADI}}</td>
                    <td>{{n.SOYADI}}</td>
                    <td>{{n.TELEFON1}}</td>
                    ...
                    <td>{{n.POZISYON}}</td>
                    <td>
                      <a class='btn btn-danger btn-sm sil text-white' @click='Delete(n.Kisi_ID,index)'><i class='fas fa-trash'></i>Sil</a>
                      <a class='btn btn-primary btn-sm edit text-white' data-toggle='modal' data-target='#modal-xl' @click='Edit(n.Kisi_ID,index)'><i class='fas fa-edit'></i>Güncelle</a>
                    </td>
                  </tr>
              ...
              <div class='row'>
                ...
                <div class='col-md-12'>
                  ...
                  <div class='card card-primary'>
                    <div class='card-header'>
                      <h3 class='card-title'> {{ model.Kisi_ID==null?'Cari Yetkili Ekle':'Cari Yetkili Düzenle' }}</h3>
                    </div>
                    <div class=''>
                      <div class='card-body'>
                        ...
                        <div class="row">
                          <div class="col-sm-3">
                            <div class='form-group'>
                              <input type='hidden' v-model='model.Kisi_ID' class='form-control form-control-sm' placeholder='Kisi_ID'>

                              <label for='CARI_ID'>Cari</label>
                              <select class="custom-select" v-model="model.CARI_ID">
                                <option v-for="(n) in CariList" v-bind:value="n.Key">
                                  {{n.Value}}
                                </option>
                              </select>
                            </div>
    ...
    methods:
    {
      ...
      async Save()
      {
        try
        {
          ...
          const res = await fetch(config.API_LOCATION + 'CARIKISILER/Create', options);
          if (!res.ok)
          {
            const message = 'An error has occured: ${res.status} - ${res.statusText}';
            throw new Error(message);
      ...
      async Edit(Kisi_ID)
      {
        ...
        try {
          const res = await fetch(config.API_LOCATION + 'CARIKISILER?ID=' + Kisi_ID, options);
          if (!res.ok) {
            const message = 'An error has occured: ${res.status} - ${res.statusText}';
            throw new Error(message);
          }
      ...
      async Update()
      {
        ...
        try
        {
          ...
          const res = await fetch(config.API_LOCATION + 'CARIKISILER', options);
          if (!res.ok) {
            const message = 'An error has occured: ${res.status} - ${res.statusText}';
            throw new Error(message);
          }
      ...
      async List()
      {
        ...
        try {
          ...
          const res = await fetch(config.API_LOCATION + 'CARIKISILER/getAll', options);
          if (!res.ok) {
            const message = 'An error has occured: ${res.status} - ${res.statusText}';
            throw new Error(message);
          }
          const data = await res.json();
          this.CARIKISILERList = data;
          this.setDatatable();
      ...
      async Deletemethod(Kisi_ID, index)
      {
        ...
        try {
          ...
          if (data == "OK") {
            $('#example1').DataTable().destroy();
            this.CARIKISILERList.splice(index, 1);
