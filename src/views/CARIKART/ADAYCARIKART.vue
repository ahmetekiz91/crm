            ...
            <div class='row'>
              <div class='col-5'>
                <button type='button' class='btn btn-primary btn-sm' data-toggle='modal' data-target='#modal-xl' @click='Temizle()'>
                  <i class='fa fa-plus'></i> Aday Cari Ekle
                </button>
              </div>
              <div class='col-4'><h3 class=''><b>Aday Cariler</b> </h3> </div>
              <div class='col-3'></div>
            </div>
          </div>
          <div class='card-body'>
            ...
            <div class="table-responsive">
              <table id='example1' class='table table-bordered table-striped'>
                ...
                <tbody>
                  <tr v-for='(n,index) in ADAYCARIKARTList'>
                    <td>{{n.CARI_KODU}}</td>
                    <td>{{n.CARI_UNVAN}}</td>
                    <td>{{n.GRUPADI}}</td>
                    <td>{{n.TEL1}}</td>
                    <td>{{n.EMAIL}}</td>
                    <td>{{n.FAX}}</td>
                    <td>{{n.ACTIVE==true?"Aktif":"Pasif"}}{{n.CARIKARTMI}}</td>
                    <td>
                      <a class='btn btn-danger btn-sm sil text-white' v-if="(n.CARIKARTMI!=1)" @click='Delete(n.CARI_ID,index)'><i class='fas fa-trash'></i>Sil</a>
                      <a class='btn btn-primary btn-sm edit text-white' v-if="(n.CARIKARTMI!=1)" data-toggle='modal' data-target='#modal-xl' @click='Edit(n.CARI_ID,index)'><i class='fas fa-edit'></i>Güncelle</a>
                      <a class='btn btn-warning btn-sm edit text-white' v-if="(n.CARIKARTMI!=1)" data-toggle='modal' @click='CariyeDonustur(n.CARI_ID)'><i class='fas fa-check'></i>Cari Karta Dönüştür</a>
                    </td>
                  </tr>
              ...
              <div class='row'>
                ...
                <div class='col-md-12'>
                  ...
                  <div class='card card-primary'>
                    <div class='card-header'>
                      <h3 class='card-title'> {{ model.CARI_ID==null?'Aday Cari Ekle':'Aday Cari Düzenle' }}</h3>
                    </div>
                    <div class=''>
                      <div class='card-body'>
                        <div class="row">
                          ...
                          <div class="col-sm-6">
                            <div class='form-group'>
                              ...
                              <select class="custom-select custom-select-sm" v-model="model.GRUPID">
                                <option v-for="(n) in GrupList" v-bind:value="n.Key">
                                  {{n.Value}}
                                </option>
                              </select>
    ...
    created()
    {
      ...
      async ddl_CARIGRUPLAR()
      {
        ...
        try
        {
          ...
          const res = await fetch(config.API_LOCATION + 'ADAYCARIKART/Create', options);
          if (!res.ok) {
            const message = 'An error has occured: ${res.status} - ${res.statusText}';
            throw new Error(message);
          }
        ...
        async CariyeDonustur(CARI_ID)
        {
          this.$dialog.confirm('Bu aday cariyi  cari karta dönüştürmek istediğinizden emin misiniz', {
            okText: "Evet",
            cancelText: 'Hayır'
          }).then((dialog) => {
            this.CariyeDonusturMethod(CARI_ID);
          })
            .catch(function (err) {
        ...
        async CariyeDonusturMethod(CARI_ID)
        {
        ...
        try
        {
          ...
          const res = await fetch(config.API_LOCATION + 'ADAYCARIKART/CariyeDonustur?CARI_ID=' + CARI_ID + '&DONUSR_ID=' + this.$store.state.auth.user.USR_ID, options);
          if (!res.ok) {
            const message = 'An error has occured: ${res.status} - ${res.statusText}';
            throw new Error(message);
          }
        ...
        try {
          const res = await fetch(config.API_LOCATION + 'ADAYCARIKART?ID=' + CARI_ID, options);
          if (!res.ok) {
            const message = 'An error has occured: ${res.status} - ${res.statusText}';
            throw new Error(message);
          }
      ...
      async Update() {
        try {
          ...
          const res = await fetch(config.API_LOCATION + 'ADAYCARIKART', options);
          if (!res.ok) {
            const message = 'An error has occured: ${res.status} - ${res.statusText}';
            throw new Error(message);
          }
      ...
      async List()
      {
        try
        {
          ...
          const res = await fetch(config.API_LOCATION + 'ADAYCARIKART/getAll', options);
          if (!res.ok) {
            const message = 'An error has occured: ${res.status} - ${res.statusText}';
            throw new Error(message);
          }
          const data = await res.json();
          this.ADAYCARIKARTList = data;
          this.setDatatable();
      ...
      async Deletemethod(CARI_ID, index) {
        ...
        try {
          ...
          const data = await res.json();
          if (data == "OK") {
            this.ADAYCARIKARTList.splice(index, 1);
            $('#example1').DataTable().destroy();
            this.setDatatable();
