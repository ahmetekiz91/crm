            ...
            <div class="card-body">
              <div class="row">
                ...
                <div class="col-sm-3">
                  <div class="form-group">
                    <label>&nbsp;</label><br />
                    <button type='button' class='btn btn-primary btn-sm btn-block' v-on:click='CARIHAREKETREPORT()'>Listele</button>
    ...
    methods:
    {
      ...
      async CARIHAREKETREPORT()
      {
        ...
        try {
          ...
          const res = await fetch(config.API_LOCATION + "CARIREPORT/CARIHAREKETREPORT", options);
          if (!res.ok) {
            this.loading = false;
            const message = 'An error has occured: ${res.status} - ${res.statusText}';
            throw new Error(message);
